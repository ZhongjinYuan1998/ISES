# -*- coding: utf-8 -*-
from textClassfication.ALBERTService import ALBERTService
from textClassfication_shixu.ALBERTService import ALBERTService as ALBERTService_shixu
from preprocess.preprocess_sentences import filter_sentences
from relationExtraction.relationExtraction import relationExtraction
import time
...
if __name__ == "__main__":
    ...
    # text = "当客户从选项菜单中选择交易类型时，自动取款机就会启动交易。自动取款机将要求客户提供适当的详细信息（如涉及的账户、金额）。然后，自动取款机将客户卡上的信息和客户输入的PIN码发送给银行。如果银行报告客户的PIN码有效，银行将批准交易，自动取款机会执行完成交易所需的任何步骤（如发放现金或接受信封），然后打印收据。然后自动取款机会询问客户是否希望进行另一笔交易。如果银行报告客户的PIN码无效，自动取款机将执行PIN码无效扩展，然后尝试继续交易。如果PIN码无效次数过多，客户的卡将被扣留，交易终止，客户将无法选择再次交易。如果交易被客户取消，或因PIN码无效次数过多以外的原因导致交易失败，自动取款机将通过屏幕告知客户交易失败的原因，然后询问客户是否希望进行另一笔交易。客户可按'取消'键取消交易，具体操作如下。所有发给银行的信息和回复都会被记录在自动取款机的日志中。"
    # text = "线上购物商城是一种便捷的购物方式。用户选定商品后，可以立即购买，马上进行结算；也可以加入购物车，稍后进行结算。用户结算后，需要提供收货地址，商城将生成商品订单。除了收货地址，商品订单中还包含商品名称、下单时间、支付价格信息。接着，支付平台将生成支付订单，并要求获取支付方式。商城会向用户展示支付方式，用户可以选择一种支付方式进行支付。支付平台会接收支付结果。若支付成功，支付平台将更新支付状态，同时配送平台会生成配送订单，然后根据配送订单配送商品，配送结果将在线上购物商城显示。若支付失败，支付平台仅更新支付状态，用户需要重新进行支付。在支付平台更新支付状态后，商城将向用户展示支付结果。用户可以根据配送结果在收货地址拿取商品，完成购物。"
    # text = "用户启动撤回安置申请程序。用户输入学生编号。用户提交撤销申请。用户输入卡号文件。"
    # text = "教师可以通过本功能参与会议活动。教师访问参加会议页面。系统显示可参加会议的列表。会议列表中包含会议时间、会议地点、主讲人信息。教师选择要参加的会议，支付参会费用。系统验证支付情况，同时向教师发送电子邮件进行参会信息确认。"
    ...
    # print("relevant_texts",relevant_texts,"shixu",shixu)

    '''
        数据预处理：分句；停用词、无用的状语去除
    '''
    filter_texts = filter_sentences(relevant_texts)
    # print("filter_texts", filter_texts)
